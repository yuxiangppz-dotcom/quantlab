...
个人日频 A 股机器学习研究与模拟账户工具。主线是：**TuShare → 来源与规范数据 → 共享特征 → 滚动训练 → 组合/账本 → 回测与评价（可选每日模拟）**。
当前支持本地研究和模拟；没有券商交易权限。工程验证、真实数据验收与策略有效性分别记录。

当前使用方式是**开机时按需研究和回测**，无需每天开机。日频指策略在历史交易日上的决策频率，
多年历史可批量回放。每日模拟、定时器和连续运行演练是以后启用前向模拟时的可选阶段。
可持续维护依靠来源留档、冻结研究协议、断点恢复和备份，不要求电脑持续在线。
...
[本地 Codex 任务](docs/ml_v2_local_completion_prompt_zh.md) 列出当前研究验收及可选模拟部署工作。
...
- **每日模拟（可选）**：模型先注册/启用；每天接收一份只读数据快照，结算昨天封存的股数订单，
...
真实数据只读接入、费用/公司行动核验与真实样本对账在本地验收。
WSL 定时任务和前向观察仅在启用持续模拟后验收，不阻断当前按需研究。
