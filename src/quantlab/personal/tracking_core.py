...
def _aware(value: str, field: str, row: int) -> datetime:
    try:
        parsed = datetime.fromisoformat(value)
    except ValueError as exc:
        raise ValueError(f"row {row} {field} must be ISO-8601") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ValueError(f"row {row} {field} must include timezone")
...
def _cash_flow_payload(event: ExternalCashFlow) -> dict:
    reported = event.reported_at or event.effective_at
    return {
...
def _cash_flow_from_payload(item: dict) -> ExternalCashFlow:
    ...
    if "effective_at" in item:
        try:
            quality = CashFlowTimingQuality(item["timing_quality"])
            effective_at = datetime.fromisoformat(item["effective_at"])
            reported_at = datetime.fromisoformat(item["reported_at"])
        except (KeyError, TypeError, ValueError) as exc:
            raise ValueError("v3 cash-flow timing payload is invalid") from exc
    ...
    return ExternalCashFlow(
        ...
        reported_at=legacy_at,
        timing_quality=CashFlowTimingQuality.LEGACY_REPORTED_AS_EFFECTIVE_UNVERIFIED,
        account_id=item["account_id"],
        direction=CashFlowDirection(item["direction"]),
        amount_fen=item["amount_fen"],
        source_sha256=item["source_sha256"],
...
def _economic_time(event: ManualFillImported | ExternalCashFlow) -> datetime:
    return event.effective_at if isinstance(event, ExternalCashFlow) else event.occurred_at
...
def _decode_journal(
    ...
    ordered = sorted((*fills, *flows), key=lambda event: (_economic_time(event), event.event_id))
...
def _parse_cash_flow_events(raw: bytes, account: dict) -> list[ExternalCashFlow]:
    ...
    for index, row in enumerate(reader, start=1):
        ...
        effective_at = _aware(row["effective_at"].strip(), "effective_at", index)
        reported_at = _aware(row["reported_at"].strip(), "reported_at", index)
        if reported_at < effective_at:
            raise ValueError(f"row {index} reported_at cannot precede effective_at")
...
def _replay_tracking(
    ...
    for event in combined:
        event_time = _economic_time(event)
        if event_time < opening_at:
        ...
        if cash < 0:
            raise ValueError(
                f"external withdrawal {event.flow_id} exceeds settled cash at its effective time"
            )
        ledger = ExecutionLedger(
            AccountSnapshot(account["account_id"], event.effective_at, cash, ledger.lots),
            calendar=calendar,
        )
    latest_economic = max(
        (_economic_time(event) for event in combined),
        default=opening_at,
    )
    latest_reported = max(
        (_reported_time(event) for event in combined),
        default=opening_at,
    )
    return _TrackingReplay(
        ledger,
        tuple(fills),
        tuple(flows),
        latest_economic,
...
def _timing_quality(flows: tuple[ExternalCashFlow, ...]) -> tuple[str, bool]:
    if not flows:
        return _NO_CASH_FLOW_TIMING, True
    if all(event.is_performance_timing_eligible for event in flows):
        return CashFlowTimingQuality.EXACT_EFFECTIVE_TIME.value, True
...
def _summary(account: dict, replay: _TrackingReplay, duplicate_count: int) -> dict:
    ...
    state = {
        ...
        "as_of": replay.latest_economic_at.isoformat(),
        ...
        state["cash_flows"] = [
            {
                "flow_id": event.flow_id,
                "effective_at": event.effective_at.isoformat(),
                "reported_at": (event.reported_at or event.effective_at).isoformat(),
                "direction": event.direction.value,
                "amount_fen": event.amount_fen,
                "timing_quality": event.timing_quality.value,
            }
    ...
    latest_flow_reported = max(
        ((event.reported_at or event.effective_at) for event in replay.cash_flows),
        default=None,
    )
    return {
        "account_id": account["account_id"],
        ...
        "as_of": replay.latest_economic_at.isoformat(),
        "latest_economic_fact_at": replay.latest_economic_at.isoformat(),
        "latest_reported_at": replay.latest_reported_at.isoformat(),
        "cash_fen": replay.ledger.cash_fen,
        ...
        "latest_external_cash_flow_at": (
            latest_flow_effective.isoformat() if latest_flow_effective else None
        ),
        "latest_external_cash_flow_effective_at": (
            latest_flow_effective.isoformat() if latest_flow_effective else None
        ),
        "latest_external_cash_flow_reported_at": (
            latest_flow_reported.isoformat() if latest_flow_reported else None
        ),
        "positions": positions,
...
def _preview_fills_internal(
    ...
    *,
    ...
    incoming = _parse_fill_events(
        ...
        account,
        calendar,
        {item.instrument_id for item in storage.load_securities()},
    )
...
def preview_manual_cash_flows(
    ...
    *,
    ...
    """Validate exact-timed deposits/withdrawals without writing account truth."""
    storage = storage or ParquetStorage(PROJECT_ROOT / "data" / "canonical")
...
def import_manual_cash_flows(
    ...
    *,
    ...
    """Atomically commit exact-timed external cash-flow facts."""
    storage = storage or ParquetStorage(PROJECT_ROOT / "data" / "canonical")
...
def load_tracking_summary(
    ...
    *,
    ...
    summary["journal_schema"] = journal.get("schema") if journal else None
    return summary
...
def load_effective_account(
    ...
    *,
    ...
    return {
        "schema": "quantlab_tracked_account_view_v2",
        "account_id": account_id,
        "account_mode": account["account_mode"],
        "as_of": summary["as_of"],
        "latest_economic_fact_at": summary["latest_economic_fact_at"],
        ...
        "latest_external_cash_flow_effective_at": summary[
            "latest_external_cash_flow_effective_at"
        ],
        "latest_external_cash_flow_reported_at": summary[
            "latest_external_cash_flow_reported_at"
        ],
        "external_cash_flow_net_fen": summary["external_cash_flow_net_fen"],
        "cash_flow_timing_quality": summary["cash_flow_timing_quality"],
...
def build_tracking_valuation(
    ...
    *,
    ...
    replay = _replay_tracking(account, _calendar(storage), fills, flows)
    last_trade_date = max((event.trade_date for event in fills), default=date.min)
    last_flow_date = max((exchange_date(event.effective_at) for event in flows), default=date.min)
    ...
    base = {
        ...
        "latest_reported_at": replay.latest_reported_at.isoformat(),
        "latest_external_cash_flow_at": (
            max(event.effective_at for event in flows).isoformat() if flows else None
        ),
        "latest_external_cash_flow_reported_at": (
            max((event.reported_at or event.effective_at) for event in flows).isoformat()
            if flows
            else None
        ),
        "cash_flow_timing_quality": timing_quality,
        ...
        return {
            **base,
            ...
            "limitations": [
                ...
                (
                    "legacy cash-flow timing is not performance-grade"
                    if not timing_eligible
                    else "cash-flow timing is explicit but TWR is not implemented"
                ),
