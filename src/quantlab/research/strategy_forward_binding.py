...
def _safe_repo_relative_ref(value: object) -> str:
    ...
    if parsed.is_absolute() or ".." in parsed.parts or parsed.parts[0] == ".":
        raise DataValidationError("forward_config_ref must be a safe repository-relative path")
    return ref
...
def validate_strategy_forward_binding(
    ...
    try:
        ...
        raise DataValidationError("strategy/forward binding input is unreadable or invalid JSON") from exc

    if raw_registry.get("schema") != "quantlab_strategy_registry_v1":
        raise DataValidationError("unsupported strategy registry schema for forward binding")
    ...
    if not isinstance(raw_entries, list) or len(raw_entries) != len(entries):
        raise DataValidationError("strategy registry entries changed during binding validation")

    forward_ref = _safe_repo_relative_ref(raw_registry.get("forward_config_ref"))
    repo_root = registry_path.parent.parent
    expected_forward_path = (repo_root / Path(*PurePosixPath(forward_ref).parts)).resolve()
    ...
    if any(forward_ref not in entry.evidence_refs for entry in entries if entry.status in _FORWARD_STATES):
        raise DataValidationError(
            "every forward-observation strategy must reference the configured Forward Shadow file"
        )

    config_id = _nonempty(forward_config.get("config_id"), "Forward Shadow config_id")
    ...
    if not isinstance(models, list) or not models or any(not isinstance(item, dict) for item in models):
        raise DataValidationError("Forward Shadow config models must be a non-empty object list")

    model_by_key: dict[tuple[str, str], dict] = {}
    for model in models:
        model_id = _nonempty(model.get("model_id"), "Forward Shadow model_id")
        version = _nonempty(model.get("version"), "Forward Shadow model version")
        source = _nonempty(model.get("source_column"), "Forward Shadow source_column")
        direction = _nonempty(model.get("direction"), "Forward Shadow direction")
        if direction not in {"higher_is_better", "lower_is_better"}:
            raise DataValidationError(f"unsupported Forward Shadow direction: {direction!r}")
        key = (model_id, version)
        if key in model_by_key:
            raise DataValidationError(f"duplicate Forward Shadow model identity: {key}")
        model_by_key[key] = {**model, "source_column": source, "direction": direction}
    ...
    for raw in raw_entries:
        ...
        if key in raw_by_key:
            raise DataValidationError("duplicate strategy entry during forward binding")
        raw_by_key[key] = raw
    ...
    for entry in entries:
        ...
        if raw is None:
            raise DataValidationError("typed strategy entry has no matching raw registry object")
        forward_model_id = raw.get("forward_model_id")
        if entry.status not in _FORWARD_STATES:
            if forward_model_id is not None and not isinstance(forward_model_id, str):
                raise DataValidationError("forward_model_id must be a string when declared")
            continue

        model_id = _nonempty(forward_model_id, "forward-observation strategy forward_model_id")
        ...
        if model is None:
            raise DataValidationError(
                ...
                f"{entry.strategy_id}@{entry.version} -> {model_id}@{entry.version}"
            )
        if model_key in claimed_models:
            raise DataValidationError(f"Forward Shadow model is claimed by multiple strategies: {model_key}")
    ...
    if claimed_models != model_keys:
        ...
        raise DataValidationError(
            f"strategy/Forward Shadow model ownership mismatch: orphaned={orphaned}, missing={missing}"
        )

    ordered = tuple(sorted(bindings, key=lambda item: (item.strategy_id, item.strategy_version)))
    fingerprint_payload = {
