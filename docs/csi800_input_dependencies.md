...
新增 `scripts/compile_csi800_membership_from_notices.py`。2016 年末两份当期
中证800指数基金年报分别披露 750 和 788 只指数投资持股，合并恰为 800 只，
且与独立的 2017-01-03 沪深300＋中证500 样本并集一致。较晚年报于
2017-03-30 公开，因此锚点只能从 2017-04-01 开始供本项目研究使用，
不能用于 2016 年末或 2017 年一季度的当时决策。

编译器核验锚点、官方公告及附件复核输入、一次性完整公告索引、证券改码
公告和 116 份月度观察的哈希。由官方生效日事件从锚点向前推演；月度观察
仅作端点对照，不决定月初成员。302132.SZ 出现在改码前月度观察的历史
回填由 2025-02-17 发行人改码生效日还原为 300114.SZ。每次事件必须满足
调出已在池内、调入尚未在池内、800 只不变、公告早于生效日；每个观察端点
须与重建结果完全一致。公告只给日期时，`known_at` 保守定为次日 18:00。

本地输出位于 `data/evidence/membership_notice_chain_v3_20260924/`，旧版及
原始观察均保留。当前项目策略配置已指向新文件；实际 universe 已越过成员
检查并在 ST 覆盖校验处停止。公告 API 本次单页返回 4202 条且 ID 全部唯一，
补回先前分页漏掉的 7 条；该 API 本身不能证明官网从未遗漏任何历史公告。
因此产物记录检索范围和这一限制，不把“月度端点一致”单独当作月内完整性证明。

## 2026-09-24 历史 ST 名称与日列表复核

新增 `scripts/certify_csi800_st_coverage.py`，只对历史中证800研究和账本可能
持有的证券范围出具 ST 覆盖文件。2354 个日级响应的原始行与 Canonical 一致，
1474 只曾入池证券的历史简称接口全部返回成功；349 次 ST 状态变化中 338 次
已有发行人公告正文核验，其余 11 次按历史简称、上市/暂停上市事实逐项保留。
独立 BaoStock 的 106 个更新日在曾入池证券范围对照，除 000939.SZ 于
2019-05-13 的漏行外一致。按已上市期间过滤，日列表与明确的风险简称只有
4 个不符日期，其中 2 个在 2018 年前，研究区间内另有 600074.SH 于
2020-01-03 漏行。

新覆盖文件把这两条研究区间内的漏行列为有来源的**加法修正**，由 universe
在读取已封存日列表后合并；不修改原始响应和 Canonical，不以一日缺行推断
摘帽。针对性端到端测试验证受影响股票当日 `must_exit=true`、不能新开，
下一日按实际状态恢复。文件仅声明历史中证800范围及日终决策；不主张
全市场 ST 完整性或精确到分钟的公告时间。此后 universe 已越过成员和 ST
两道门槛，下一处实际阻断是行业历史可得时间。

## 2026-09-24 申万分类版本校正

供应商 `index_member(ts_code=...)` 将 SW2021 新增的一级行业成员关系追溯到
分类推出前；旧转换按原始 `in_date` 判断版本，并在旧版无对应代码时退回
新版名称，导致 2018—2021 的真实旧版分类与追溯新版分类重叠。
`_versioned_sw_stints` 现按申万官方公布的指数切换日 2021-12-13 裁剪
SW2014/SW2021 各自适用期；两版共用代码且名称不同的记录在该日拆开。
2014版指数于 2021-12-13 停止发布，新版自同日起发布，见申万宏源研究
[2021-11-23《2021版申万行业指数切换公告》](https://www.swsresearch.com/institute-sw/api/index_announcements/details/?id=43CE85B9-E7FC-439B-A9F8-18A2492AAD5A)。与之不同，新版**分类结果**
已在 2021-07-31 宣布公开、每周更新；研究这里保守采用较晚的指数切换日
作为统一分类版本边界。

新文件在 `data/evidence/industry_taxonomy_eras_20260924/`，原文件未覆盖。
成员交易日中的行业内容未知从 41,572 降为 48；剩余包括 2018 年末
000979.SZ 的 21 日、2021 年两日全市场备用列表空响应导致的部分科创板
证券，以及 600068.SH 合并前停牌窗口。它们仍保留未知。版本校正仅修复
**分类内容**；个人历史可得时间仍为 `known_at=null`，不能据此让 universe
放行或宣布行业 PIT 已验收。
