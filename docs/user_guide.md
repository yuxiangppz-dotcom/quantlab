...
新增的“全年训练频率对照：执行进度”保留较长区间的周更/低频对照。
本批次在完成94次新训练后，旧模型扩展复用的数值校验失败并停止；已完成部分核验通过，
软件修复不等于整批研究完成。进度、失败及剩余工作见
[本轮失败与修复说明](extended_frequency_reuse_repair_zh.md)。下方旧六次比较仍是独立的历史批次。
随后完成的“旧模型恢复验证：独立记录”已核验六个旧引用，新增训练为0；合并覆盖100 / 104。
使用方法和精确边界见[旧模型恢复中文说明](extended_recovery_findings_zh.md)。原失败批次与恢复记录分别保留。
