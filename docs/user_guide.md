...
“回测与基准”中的“Qlib 因子验证：Alpha158”显示本轮 158 个原生因子的验证与可用历史。
展开因子表与代码覆盖可查看缺失边界，并下载 [中文解读](alpha158_findings_zh.md)。
验证通过表示计算和数据边界经过检查，不表示因子已经证明能赚钱。

同页的“Alpha158 历史数据：滚动训练准备”显示较大历史股票范围的生成进度和实际覆盖。
只有完整报告通过校验才显示数据已就绪；可查看按年记录、因子缺失和资源使用，并下载
[历史数据中文解读](alpha158_history_findings_zh.md)。该生成批次不含训练。

同页上方的“Alpha158 滚动模型：六次固定比较”展示已完成的三个时间窗口、岭回归和
LightGBM 共六次训练。先看评价年份的日均 RankIC，再对照训练期差距、预测/有效标签
记录及名单变化；可下载完整记录和 [模型中文解读](alpha158_rolling_findings_zh.md)。
2026 年复用最后一折模型，没有重训。结果属于已观察历史的诊断，RankIC 不是收益率，
名单变化不是实际换手。当前策略和账户不会因该页面的模型结果而自动改变。

“全年训练频率对照：完整结果”已完成104个模型引用、全部4,320,562条预测的独立复核。
查看[完整年度中文解读](extended_annual_findings_zh.md)：本段历史的周更没有带来平均排序提升，
2026年的信号也较弱，目前没有稳定盈利或20%回撤达标结论。
页面提供全部摘要和诊断下载；原94次训练后的失败、六个旧引用恢复及最后四个名额分别保留。
较早的六次模型比较仍是独立历史批次。
...
左侧打开“组合与成本研究”，选择5/10/20交易日调仓间隔与额外单边摩擦，先看五组路径的
共同有效区间，再选择具体策略和假设资金，检查自身有效截止、停止原因、持仓、现金与成本。
切换选项只读取保存结果，不重新训练或执行回测；页面可下载全部60条情景、逐日持仓、
价值转移及独立复核。

当前60条均已复核，16条覆盖完整观察期、44条因生命周期证据不足而停止。
16条完整路径的研究回撤约30.3%—42.1%，未满足20%研究目标。不同有效截止日期不能直接比较；
完整解读见 `docs/extended_economic_findings_zh.md`。这里的收益只扣声明额外摩擦，
不含已核验的逐单最低佣金、印花税和权益现金账；假设资金是线性缩放，不是实际账户或实盘批准。
