...
- Integrate the binding into Strategy Readiness so readiness never assumes `strategy_id == forward_model_id`.
...
- Strategy Readiness can consume validated bindings and correctly resolves the current baseline name mismatch.
