...
Remove the remaining duplicate Daily target-selection and weight-allocation implementation from the Daily calculation path and make the materialized Daily target derive from the canonical fixed-count portfolio constructor already used by Forward Shadow and product-aligned research audit.
...
- Preserve the immutable content-addressed publication facade added before this task.
...
- Self-review confirms no duplicated economic selection/weight logic remains in the Daily calculation path.
