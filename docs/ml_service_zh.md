...
标价 JSON 格式沿用回放的 initial-marks。空仓初始化可以包含全市场报价，首日仅保留当日输入范围；共同证券的价格必须一致，不能放宽价格冲突检查。初始化后不修改本金、策略参数及历史状态；不同政策开独立服务。
...
可在当日记录为 decision_blocked，但已完成的账户结算保留。**账本每天只提交一次，决策可在截止前恢复**：
使用相同日期、输入和代码重跑 `daily` / `run-day`，在当天仍为账户最新日期、尚无最终计划且未暂停时，
会追加一次决策尝试。模型恢复或暂停解除后无需等到下一交易日。重试不再次结算，不改原账本、不改本金。

重试记录位于 `decisions/日期/0001...`，预测位于 `retry_signals/日期/0001...`；每次尝试绑定原账本、
输入、代码、截止政策和前一次尝试。失败尝试保留；第一份成功计划冻结后，再运行只返回同一结果。
下一日账本绑定真正消费的决策及其发布时间指纹，UI 和报告也读取恢复后的决策。超时、历史补跑、
账户已经推进到后续日期时均不能补造订单。发布中断后以实际恢复时刻记录，不能倒签。
已封存输入不支持原地修订；若事实需要修正，应停止并按来源对账流程处理，不能删 hash/回执绕过。
操作上仍应先完成数据/模型预检，再触发正式日任务。
